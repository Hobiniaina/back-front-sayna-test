...
exports.register = async (req, res) => {
	let datas = req.body;
	// Validate request
	if (!req.body.firstname || !req.body.lastname || !req.body.Email || !req.body.sexe || !req.body.Password || !req.body.date_naissance || !req.body.sexe) {
		res.status(401).send({
			error: true,
...
exports.deleteUser = async (req, res) => {
	try {
		let id = "";
		let idUsers = "";
		await db.collection('tokens').where('token', '==', checkTokenMiddleware.extractBearerTokens(req.headers.authorization))
			.get()
			.then((querySnapshot) => {
				querySnapshot.forEach((doc) => {
					id = doc.id;
					idUsers = doc.data().userId
				})
			})
		await db.collection('users').doc(idUsers).delete();
		res.status(200).send({
			error: false,
			message: "Votre compte et le compte de vos enfants ont été supprimésavec succès"
		});
	} catch (e) {
		res.status(500).send({
			error: true,
